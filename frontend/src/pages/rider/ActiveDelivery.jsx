      ...
      <div style={{ marginLeft: isMobile ? 0 : '280px', flex: 1 }}>
        <div className="d-flex justify-content-between align-items-center px-4 py-3"
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <h5 className="fw-bold mb-0">Active Delivery</h5>
            <div className="d-flex align-items-center gap-1">
              <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: connected ? '#198754' : '#dc3545' }}></div>
              <small className="text-muted">{connected ? 'Live' : 'Connecting...'}</small>
            </div>
          </div>
          <div className="d-flex align-items-center gap-3">
            ...
            <div className="col-lg-8">
              <div className="card border-0 shadow-sm" style={{ borderRadius: '16px', overflow: 'hidden' }}>
                <MapContainer
                  ...
                  zoom={14} style={{ height: '450px', width: '100%' }}>
                  <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution='© OpenStreetMap contributors' />
                  {order?.dropoff_lat && (
                    <Marker position={[parseFloat(order.dropoff_lat), parseFloat(order.dropoff_lng)]} icon={dropoffIcon}>
            ...
            <div className="col-lg-4">
              ...
              <div className="card border-0 shadow-sm p-4" style={{ borderRadius: '16px' }}>
                ...
                {statusFlow[currentStatus] && (
                  <button className="btn w-100 text-white fw-bold py-3"
                    style={{ backgroundColor: statusFlow[currentStatus].color, border: 'none', borderRadius: '10px' }}
                    onClick={handleStatusUpdate}>
                    {statusFlow[currentStatus].label}
                  </button>
                )}
                {currentStatus === 'delivered' && (
                  <div className="alert alert-success text-center mt-3">✅ Delivery completed!</div>
                )}
              </div>
            </div>
          </div>
